...
def play_game(show_play, player_names, decide_functions):
    ...
    while True:
        ...
        if show_play:
            ...
            print("      First throw: <span class='die'>{}</span>".format(dice[subscore]))
        ...
        if subscore == 1:
            ...
            while decide_functions[who_to_play](scores[who_to_play], subscore, scores[other_player_index]):
                ...
                if show_play:
                    print("      Next throw: <span class='die'>{}</span>".format(dice[die_result]))
            ...
            if subscore > 0:
                if show_play:
                    print("      {} takes the loot!\n".format(player_names[who_to_play]))
...
def compete(player_names, decide_functions):
    ...
    for game in range(games):
        if game < games_to_show:
            print("\n### Example Game {}\n".format(str(game + 1) if games_to_show > 1 else ""))
            print("<pre>\n")
...
# Pig Arena Report
    ...
    print("||{}|Significant Wins<br>(95% Confidence)|".format("|".join(player_names)))
    print("|--:|{}:--:|".format(":--:|" * len(player_names)))
    for player_name_row in player_names:
        print("|**{}**|{}|{}|".format(
            player_name_row, 
            "|".join([record[player_name_row][player_name_col] for player_name_col in player_names]),
